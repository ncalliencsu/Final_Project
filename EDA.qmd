...
The 2 -Way Contingency Tables are printed for all variables against Diabetes. The reader can look at the tables and look for differences in percentages in the rows between Diabetes = 0 and Diabetes = 1.  I'm not going to analyze all that, but I put it here for the consideration of the reader who wants to see the numbers on the contingencies.
...
0 indicates no correlation between the two variables. (Statology.org)
...
hist(as.numeric(df$Age), xlab="Age", main="Histogram of Age")
...
#Grouped Bar Plots
ggplot(data = df, aes(x= Diabetes , fill = HighBP)) +
  geom_bar(position = "dodge") +
  labs(x = "Diabetes") +
  scale_fill_discrete("HighBP") +
  labs(title = "Count of Diabetes by HighBP", x = "Diabetes", y = "Count by HighBP")
...
ggplot(data = df, aes(x= Diabetes , fill = Sex )) +
  ...
  labs(title = "Count of Diabetes by Alcohol", x = "Diabetes", y = "Count by Sex")
...
mosaic(data = df, ~ HighBP + Income + Diabetes, highlighting = "Diabetes",
       highlighting_fill = c("lightblue", "pink"),
       rot_labels=c(0,90,0,0), just_labels=c("left","right"),
       main = "Mosaic Plot of Diabetes, Income and HighBP")

#Box Plots
ggplot(df, aes(x = Age , y = Diabetes )) +
  geom_boxplot() + labs(title = "Boxplot of Diabetes by Age", x = "Age", y = "Diabetes")  

# Source - https://stackoverflow.com/q/27050802
...
ggplot(df, aes(x= Age, y= Diabetes)) +
 ...
 facet_wrap(. ~ Education, scales = "free_y") +
  ...
  labs(title = "Boxplots of Diabetes by Age and Education", x = "Age", y = "Diabetes")
...
ggplot(df, aes(x= Age, y= Diabetes)) +
 ...
 facet_wrap(. ~ Income, scales = "free_y") +
  theme(axis.text.x = element_text(angle = 90, vjust=.5, hjust=1)) + 
  labs(title = "Boxplots of Diabetes by Age and Income", x = "Age", y = "Diabetes")
```
The histograms show something interesting. From inspection of the Income and Education Histograms, it appears as though there is a bias in the sample toward higher education and higher income individuals. I cannot imagine that was a desired effect, but it looks like a defect in the sampling plan. 
...
The Box Plots of Diabetes vs. Age and Income seem to show a different picture than the Mosaic Plots. Here, on the faceted Boxplot of Diabetes vs. Age and Income it is not really discernible if there is a difference plots.  I think the Mosiac Plots are the best visual tool for this dataset.
