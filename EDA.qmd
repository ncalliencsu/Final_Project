...
The goal of the EDA is to inform the modeling process in the selection of predictor variables.
...
Columns 15, 20, 21 and 22 will be converted to ordered factors.\
BMI, Mental Health and Physical Health will be left as type integer. Most columns will be renamed for readability.
...
After review of the dataset data_00, in which many variables have been converted to factors, there are no missing values and the mins and maxes seem to be reasonable. The data was advertised as clean so this is what we would expect.
...
summary(df, maxsum = 30)
```

The summary statistics function is very nice here. It shows all the counts for each factor variable by level and summary statistics for the three integer variables. Notice the Education and Income are both skewed to the higher brackets. We can also see the study has a lot more women than men in the sample. The percentages are more interesting but I cannot get them to render in HTML.
...
Tetrachoric correlation is a measure of the correlation between two binary variables. This type of correlation is often used in surveys and personality tests in which the questions being asked only have two possible response values. The value for a tetrachoric correlation can range from -1 to 1 where:
...
From inspection of the Tetrachoric Correlation Matrix, there appears to be moderate correlation between Diabetes and the following binary predictor variables:
...
From inspection of the Point-biserial correlation matrix, there appears to be moderate correlation between Diabetes and the following ordinal and continuous predictor variables:

-   BMI(22%)
-   Gen_Hlth(29%)

For all of my assessments here on correlation, I used 20% as the cutoff, which is still not a strong correlation. The strongest correlations with Diabetes from this data are High Blood Pressure, High Cholesterol, Heart Condition and Difficulty Walking. Note that all these have correlations with each other at approximately 40% levels. I am not sure if this is enough to collapse them into one variable and is a good question for Office Hours.
...
#Box Plots
ggplot(data = df, aes(x = as.numeric(Age_Group) , y = Diabetes )) +
...
ggplot(df, aes(x= as.numeric(Age_Group), y= Diabetes)) +
...
The histograms show something interesting. From inspection of the Income and Education Histograms, it appears as though there is a bias in the sample toward higher education and higher income individuals. I cannot imagine that was a desired outcome of the sampling plan but this seems to be the case.

The Bar Plots show what we expect with High_BP having an effect on Diabetes. We can also see that Sex does not seem to have much of an effect, and it could be argued that Alcohol has a positive effect on the Diabetes outcome.
...
[Click Here for Modeling Page](https://ncalliencsu.github.io/Final_Project/Modeling.html)
